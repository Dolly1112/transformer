...
def process_favorita(config):
  ...
  data_folder = config.data_folder # data_folder 是数据存储的文件夹路径，使用 config.data_folder 进行配置。
  ...
  # zip_file 指向要下载的压缩文件的路径，它位于 data_folder 的上一级目录中，以避免在数据重处理时意外删除。
  # Expected zip file path: /home/my_tft/TFT_code/favorita_dataset/data/favorita/../favorita-grocery-sales-forecasting.zip
  ...
  print('Expected zip file path:', zip_file)
  """

  # Unpack main zip file
  outputs_file = os.path.join(data_folder, 'train.csv.7z')
  unzip(zip_file, outputs_file, data_folder)
  # outputs_file 是解压后 train.csv.7z 文件的路径。unzip 函数用于将主压缩文件（zip_file）解压缩到 data_folder 中，并生成 .7z 文件。
