...
def get_decoder_mask(self_attn_inputs): # mask = get_decoder_mask(enriched)
...
class InterpretableMultiHeadAttention():
  ...
  def __init__(self, n_head, d_model, dropout):
    ...
    Args:
      n_head: Number of heads
      d_model: TFT state dimensionality # hidden_size
  ...
  def __call__(self, q, k, v, mask=None):
    ...
    Args:
      q: Query tensor of shape=(?, T, d_model) # ? denotes n_head
    ...
    for i in range(n_head):
      qs = self.qs_layers[i](q) # input_dim=q; output_dim = d_k
    ...
    outputs = self.w_o(outputs) # input_dim=d_model; output_dim=K.mean(head, axis=0)
...
class TemporalFusionTransformer(object):
  ...
  def get_tft_embeddings(self, all_inputs):
    ...
    # Make sure all_inputs.get_shape().as_list()[-1] == self.input_size
    ...
    for i in range(num_categorical_variables):
      ...
      # tf.keras.layers.Embedding:  create the embeddings to embed higher dim data into lower dim vector space.
      # can use lora_rank
    ...
    if self._static_input_loc:
      ...
      #static_inputs = tf.keras.backend.stack(static_inputs, axis=1) # concat base on axis=1
      static_inputs = [tf.expand_dims(tensor, axis=1) for tensor in static_inputs]
      static_inputs = tf.concat(static_inputs, axis=1) # concat base on axis=1
    ...
    obs_inputs = tf.keras.backend.stack([
        ...
        for i in self._input_obs_loc
    ], axis=-1) # axis=-1: last dimension

    # (3) Observed (a priori unknown) inputs
    ...
    unknown_inputs = [] # for regular_inputs & not belongs to known & not belongs to _input_obs_loc
  ...
  def _build_base_graph(self):
    ...
    encoder_steps = self.num_encoder_steps # training-testing threshold
    ...
    def static_combine_and_mask(embedding):
      ...
      _, num_static, _ = embedding.get_shape().as_list() # axis=1
      ...
      sparse_weights = K.expand_dims(sparse_weights, axis=-1) # tf.keras.backend.expand_dims:在某个位置多加一个维度，数值为1
      ...
      static_vec = K.sum(combined, axis=1) # K = tf.keras.backend
    ...
    def lstm_combine_and_mask(embedding): # use static_context_variable_selection
      ...
      expanded_static_context = K.expand_dims(
          static_context_variable_selection, axis=1) # static_context_variable_selection多加一个维度
    ...
    def get_lstm(return_state):
      ...
      if self.use_cudnn: # Simply switching the call from LSTM() to CuDNNLSTM() took less than a minute.
    ...
    history_lstm, state_h, state_c \
        = get_lstm(return_state=True)(historical_features,
                                      initial_state=[static_context_state_h, # h_0
    ...
    # Nonlinear processing on outputs(Position-wise Feed-forward Layer)
